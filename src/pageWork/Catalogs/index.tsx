...
const Catalog = () => {
  ...
  const [sortKey, setSortKey] = useState<SORT_PRODUCTS>(
  ...
  const filteredProducts = useMemo(() => {
    const sort =
      brand !== -1 ? data?.filter((item) => item?.brand === brand) : data;
              ...
              <form className="form">
                ...
                <label className="form__select">
                  <select name="category">
                    <option defaultValue="0" selected label="0" />
                    <option defaultValue="1" label="1" />
                    <option defaultValue="2" label="2" />
                  </select>
                  <span>Срок поставки</span>
                </label>
                <label className="form__select">
                  <select name="category">
                    <option defaultValue="-1" selected label="По умолчанию" />
                    <option defaultValue="0" label="Оригинал" />
                    <option defaultValue="1" label="Аналог" />
                  </select>
                  <span>Качество</span>
                </label>
